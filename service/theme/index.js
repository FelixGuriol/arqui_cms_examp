...
const rootDir = 'D:\\janaq\\tok2kit\\ecommerce\\oth\\amazon\\'
...
function configResolver(theme, page, host) {
    ...
    if(gconfig){
        gconfig.layout={}
        if(gconfig.use_header)gconfig.layout.header=getComponentOrDefault(host,theme,'header')
        if(gconfig.use_footer)gconfig.layout.footer=getComponentOrDefault(host,theme,'footer')
        return gconfig
    }
    return {}
}
